// Auto-criação dos canais de publicação/análise no boot (evento ready). Idempotente: se o ID já
// está no `estado` e o canal ainda existe, não faz nada; se falta ou o canal foi apagado, cria de
// novo, salva o ID e loga. Requer "Gerenciar Canais" — sem ela, só loga aviso e segue (NÃO derruba
// o bot). Os comandos /criar-* continuam como fallback manual. NÃO toca na env legada
// CANAL_DIARIO_OFICIAL_ID (canal de pegar-casos).
...
async function garantirUmCanal(guild, { getId, setId, nome, topic, rotulo, criar, aoCriar }) {
  ...
  const { canal, erroMsg } = await criar(guild, guild.client.user.id, { nome, topic });
...
async function garantirCanais(guild) {
  ...
  try {
    await garantirUmCanal(guild, {
      ...
      rotulo: 'Diário Oficial', criar: criarCanalReadonly,
    ...
    await garantirUmCanal(guild, {
      getId: () => estado.obter('canalEditaisId'), setId: (id) => estado.definir('canalEditaisId', id),
      nome: '📢│editais',
      topic: 'Editais de processo seletivo — inscrições pelos botões abaixo de cada edital (somente leitura).',
      rotulo: 'Canal de editais', criar: criarCanalReadonly,
    });
    await garantirUmCanal(guild, {
      getId: () => estado.obter('canalAnaliseId'), setId: (id) => estado.definir('canalAnaliseId', id),
      nome: '📋│análise-inscrições',
      topic: 'Análise de inscrições de editais (staff) — cards automáticos; decida em Aprovar/Reprovar.',
      rotulo: 'Canal de análise', criar: criarCanalStaff,
    });
  } catch (e) {
