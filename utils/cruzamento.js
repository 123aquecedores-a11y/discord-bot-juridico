...
// Discord do CLIENTE, não de quem preencheu. Por isso tudo aqui é indexado por CPF: usa todo
// ID de Discord já vinculado à ficha desse CPF (a mesma pessoa pode ter usado contas
...
function idsConhecidosDoCPF(cpf, discordIdInformado) {
  const registro = ficha.buscarPorCPF(cpf);
  return [...new Set([...(registro?.discordIds || []), discordIdInformado].filter(Boolean))];
}

function verificarAntecedentesPorCPF(cpf, discordIdInformado) {
  const ids = idsConhecidosDoCPF(cpf, discordIdInformado);
  if (ids.length === 0) {
...
function resumoTextoPorCPF(cpf, discordIdInformado) {
  const r = verificarAntecedentesPorCPF(cpf, discordIdInformado);
...
// em qualquer conta de Discord já vinculada a esse CPF.
function temNovoAntecedenteEmPorCPF(cpf, discordIdInformado, dias) {
...
module.exports = { verificarAntecedentesPorCPF, resumoTextoPorCPF, temNovoAntecedenteEmPorCPF };
